...
?>
<div class="spi-ledger mb-4">
    <?php foreach ($items as $item): ?>
        <div class="spi-ledger-item">
            <div class="spi-ledger-label"><?= $item['label'] ?></div>
            <div class="spi-ledger-value <?= $item['class'] ?? '' ?>"><?= $item['value'] ?></div>
        </div>
    <?php endforeach; ?>
    <?php if (!empty($invoice->detail)): ?>
        <div class="spi-ledger-item" style="grid-column:1 / -1;">
            <div class="spi-ledger-label">Detalle</div>
            <div class="spi-ledger-value" style="white-space:normal;"><?= h($invoice->detail) ?></div>
        </div>
    <?php endif; ?>
</div>
