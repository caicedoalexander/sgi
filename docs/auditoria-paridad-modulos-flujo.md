...
> **▶ Próximo paso al retomar:** terminar lo que queda de **Ola 3c** — (1) `PettyCash/edit` → `sgi-edit-shell` + reusar `element('pipeline_sidebar')` (restructura del grid Bootstrap `row g-3`/`col-lg-4/8`; la pieza más grande y de mayor riesgo, verificar visualmente con Playwright), y (2) docs especiales de `Advances/legalization` (grupos con firmas: relación de facturas / consignación / historial de firmas → evaluar pasarlos como `groups` a `documents_section`). Verificar visualmente las vistas de los demás módulos cuando haya registros (timeline de Novelty, etc.). Luego **Ola 4** (limpieza de dead code) y **Ola 5** (fixes del canon Invoice + rename `*Service`→`*PipelineService`). El sub-paso de seguridad diferido (Novelty `#[Permission]`→`#[PipelineAction]`) va aparte (ver "Pendientes con revisión de seguridad").
...
### [MEDIUM] Soportes documentales: markup bespoke en vez de `element('documents_section')`
`Advances/legalization` (~160 líneas a mano), `Refunds/view+edit` (markup inline + modal bespoke), `EmployeeNovelties/edit` duplican el markup en vez de delegar al element compartido (que SÍ usan `EmployeeNovelties/view` y `NoveltyLiquidationDocs/edit`).
...
| `GroupingPipelineConstantsTrait::BACKWARD_TRANSITIONS` | trait + comentarios en `Refund/State/AutorizacionPagoState` | 0 lecturas. **NO confundir** con `PaymentSchedulingConstants::BACKWARD_TRANSITIONS` (vive). |
| `RefundPipelineState::getNextStatus()` + 6 States | `Pipeline/Refund/*` | Falso gemelo: Refund avanza con `RefundConstants::TRANSITIONS`. Borrar SOLO si se conserva `TRANSITIONS` (decisión Ola 2). |
