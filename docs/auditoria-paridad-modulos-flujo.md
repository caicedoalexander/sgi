...
- **Ola 2** ✅ (decisiones tomadas: rename completo a `*PipelineService` en Ola 5; Refund migra a `State::getNextStatus`)
  ...
  - **DIFERIDO**: Novelty — autorización por-paso en `advance`/`reject`/`advanceGroup` (plan en `docs/plan-seguridad-novelty-pipelineaction.md`; la redacción "→`#[PipelineAction]`" se **corrigió** 2026-05-29: solo `reject` cambia de atributo, `advance`/`advanceGroup` quedan en `#[Permission(edit)]` + RBAC en servicio).
...
- **Novelty: autorización por-paso en `advance`/`reject`/`advanceGroup`** — **DIFERIDO** (sesión de seguridad dedicada). **Plan completo: [`docs/plan-seguridad-novelty-pipelineaction.md`](plan-seguridad-novelty-pipelineaction.md).**
