...
- **Novelty: autorización por-paso en `advance`/`reject`/`advanceGroup`** — **DIFERIDO** (sesión de seguridad dedicada). **Plan completo: [`docs/plan-seguridad-novelty-pipelineaction.md`](plan-seguridad-novelty-pipelineaction.md).**
  ...
  - El hueco real NO es el atributo: es que `EmployeeNoveltiesController::advance` hace `patchEntity` crudo (sin `filterEntityData`) y llama `advance()` sin `roleId`/`denialReasonForAdvance` (que **ya existe**, `NoveltyPipelineService:466`, pero solo se usa para display).
  - Requiere: (1) decisiones del plan §5, (2) verificar/seed `pipeline_permissions` para `novelties`+`liquidation_docs` (lockout si faltan), (3) prueba manual por rol. Ver plan.
