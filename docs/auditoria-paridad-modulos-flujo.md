...
- **Ola 2** ✅ (decisiones tomadas: rename completo a `*PipelineService` en Ola 5; Refund migra a `State::getNextStatus`)
  ...
  - **✅ HECHO (2026-05-29)**: Novelty — autorización por-paso (`canOperate`) en `advance`/`reject`/`advanceGroup`, enforcing en el servicio. (`advance`/`advanceGroup` mantienen `#[Permission(edit)]`; `reject` → `#[PipelineAction(PIPELINE_NOVELTIES)]`. Mass-assignment en `advanceGroup` cerrado por revisión adversarial.) Pendiente solo la verificación manual por rol. Ver `docs/plan-seguridad-novelty-pipelineaction.md`.
...
- **Ola 4** ✅ (2026-05-29) — **Backend dead code ELIMINADO** (re-verificado 0-refs por mí con grep + `php -l` + 192/192): `InvoiceConstants::TRANSITIONS`, `NoveltyConstants::TRANSITIONS`, `RefundConstants::TRANSITIONS`, `RefundConstants::BACKWARD_TRANSITIONS`, `PettyCashConstants::BACKWARD_TRANSITIONS` (+ comentario stale reescrito en `Refund/State/AutorizacionPagoState`), `InvoicePipelineService::getStatusIndex()`, `InvoiceApprovalService::getActiveApprovals()` y `getApprovalSummary()` (sin orfanatos: `_summaryFromApprovals`/`getCurrentApprovals`/`getApprovalSummariesBatch` siguen vivos). **OJO — falso positivo del workflow (corregido):** `PettyCashConstants::TRANSITIONS` estaba **VIVO** (`PettyCashRecordsController:324`) → no se borró en este lote. _(Update 2026-05-29: eliminado luego al alinear PettyCash a `coordinador->getNextStatus()` vía State — ver Ola 5; ya **ningún** módulo usa mapas `TRANSITIONS`.)_ Además **elements huérfanos ELIMINADOS**: `element/copcsa.php`, `element/pipeline_progress.php` y `element/progress_stepper.php` (0 invocaciones — `progress_stepper` solo lo invocaba `pipeline_progress`, así que se borró la familia completa) + citas corregidas en `CLAUDE.md` y `arquitecture.md`. **(a) `advanceStatus` legacy ELIMINADO (2026-05-29):** usuario confirmó 0 integraciones externas → borradas las acciones `InvoicesController::advanceStatus` + `PettyCashRecordsController::advanceStatus`, sus rutas en `config/routes.php`, y `InvoicePipelineService::advance()` (único caller, vía `InvoicesController:450`). **Conservados** (vivos): `PettyCashPipelineService::advanceStatus` (vía `saveAndAdvance`) y todo `RefundPipelineService::advanceStatus` + su ruta. 192/192 + 0 refs colgantes. **(b) `FIELDS_BY_STEP`/`SECTIONS_BY_STEP`/`fieldsByStep`/`sectionsByStep`/`pipelineKey` en PettyCash/RefundFieldAccessPolicy → DECISIÓN: NO refactorizar (2026-05-29, excepción documentada).** Son inertes (esas policies override `filterEntityData` completo; `getEditableFields`/`getVisibleSections` nunca se invocan para PettyCash/Refund — verificado: solo los llaman los coordinadores Invoice/Novelty). PERO los 3 métodos son `abstract` en la base `PipelineFieldPolicy`: eliminarlos exige des-abstraer la base (degradar el contrato a concreto con default vacío) o dejar stubs `return []` (código igualmente muerto en forma). Mal trade para código inerte sin bug.
- **Ola 5** ✅ (2026-05-29) — Fixes del canon Invoice + rename `*Service`→`*PipelineService` + convergencia de verbos (todo completo). **Fixes APLICADOS (2026-05-29, php -l + 192/192):** `getNextStatus($currentStatus, $invoice->document_type)` en `InvoicesController:369` (respeta DocumentTypePolicy); `final class InvoiceFieldAccessPolicy`; comentario inline en `DIAN_REJECTED` (género divergente deliberado); `'+48 hours'` → `InvoiceConstants::APPROVAL_TOKEN_HOURS` en `InvoiceApprovalService:80` (2.ª instancia, fuera del item original). **Diferidos (no triviales):** (a) ~~magic `'approve'`/`'reject'`~~ ✅ HECHO (2026-05-29): nuevo `App\Constants\ApprovalConstants` (`ACTION_APPROVE`/`ACTION_REJECT`/`ACTIONS`), **final**, usado **solo** por los módulos con aprobación externa (Invoice + Novelty strategies + `ExternalApprovalsController` + `InvoiceApprovalService`) — NO por los pipelines de agrupación (PettyCash/Refund/PaymentScheduling/Advance no tienen acción approve/reject). 10 literales reemplazados en 4 archivos; 192/192; (b) ~~`FIELD_LABELS` `payment.*`~~ ✅ HECHO (2026-05-29): 5 claves fijas en `InvoiceHistoryService::FIELD_LABELS` (`payment.status/rejection_reason/amount/payment_date/banking_entity_id`); el set "dinámico" resultó acotado (`$changes` ⊆ amount/payment_date/banking_entity_id por el patch de `InvoicePaymentService:476`) → claves fijas bastan, aditivo, sin tocar la vista (`recordChanges` usa lista hardcodeada, no `array_keys(FIELD_LABELS)`); (c) ~~`add.php:187` literal `'aprobacion'`~~ ✅ corregido → `InvoiceConstants::STATUS_APROBACION` (2026-05-29). **✅ RENAME EJECUTADO (2026-05-29):** los 4 coordinadores renombrados a `*PipelineService` (`RefundPipelineService`, `PettyCashPipelineService`, `NoveltyPipelineService`, `PaymentSchedulingPipelineService`) vía `git mv` + reemplazo word-boundary en 29 archivos `src/` + `composer dump-autoload`; **192/192**, 0 referencias viejas; CLAUDE.md actualizado (tabla Key Services + nomenclatura). `AdvanceLegalizationService` **NO** renombrado (no es coordinador de pipeline). **✅ CONVERGENCIA DE VERBOS EJECUTADA (2026-05-29, commits separados B→A→C/C'):** (B) Novelty `validateTransition`→`validateTransitionRequirements`; (A) Refund+PettyCash `getTransitionErrors`→`validateTransitionRequirements`; (C) Refund `advanceStatus`→`advance` (acción/ruta pública conservada) + (C') PettyCash helper interno `advanceStatus`→`advance`. Vocabulario unificado del coordinador: `validateTransitionRequirements` / `saveAndAdvance` / `advance` / `regress`. **Divergencias de dominio confirmadas LEGÍTIMAS (NO se tocan):** Novelty `getNextStatus(object, $type)` (skips condicionales por tipo: salta `aprobacion`/`GDP`); Novelty `reject` (rechazo **terminal** → `RECHAZADA`, distinto de la regresión fría `regress` — Novelty no tiene `regress`); `advanceGroup`/`validateGroupTransition` (ops de lote del doc de liquidación). `saveAndAdvance` (guardar+avanzar) vs `advance` (avance puro) son operaciones distintas, no nombres divergentes. `php -l` + 192/192 en cada paso. **Alineaciones finales (2026-05-29):** (1) orden de params de `advance` de Refund → `(record, roleId, userId)` (canon, igual que PettyCash/PaymentScheduling; Novelty sigue sin `roleId` por su modelo de auth vía `#[Permission]`). (2) PettyCash gana `getNextStatus(string)` vía State y su controller deja de leer `PettyCashConstants::TRANSITIONS` (eliminada) → **ningún módulo usa ya mapas `TRANSITIONS`** (avance siempre resuelto vía State). **Blast radius original mapeado (referencia pre-rename):** renombrables = `RefundService` (8 archivos src), `PettyCashService` (10), `NoveltyService` (10, 2 controllers), `PaymentSchedulingService` (7, el menos disruptivo; ya alinea `validateTransitionRequirements`/`advance`/`regress` con Invoice). **`AdvanceLegalizationService` EXCLUIR del rename**: NO es coordinador de pipeline (Advance reusa `InvoicePipelineService`); es servicio de dominio (firmas/outcomes/refunds), 19 src + 2 tests — requiere decisión de canon Advance/AdvanceLegalization aparte. `InvoicePipelineService` ya cumple. Corrección de drift: el verbo `regressStatus` citado en el doc **no existe** (el set real de regreso es `regress`/`reject`). Rename de clase ≠ convergencia de verbos (dos tareas distintas).
...
### [HIGH] Fragmentación de nomenclatura del coordinador y sus métodos — ✅ RESUELTO (2026-05-29)
**Resuelto:** los 4 coordinadores renombrados a `*PipelineService` (Ola 5 rename) y los verbos convergidos al canon (`validateTransitionRequirements`/`saveAndAdvance`/`advance`/`regress`). Quedan como **divergencias legítimas** documentadas: `AdvanceLegalizationService` (servicio de dominio, no coordinador — Advance reusa `InvoicePipelineService`); Novelty `getNextStatus(object,$type)`, `reject` (terminal) y `advanceGroup`/`validateGroupTransition` (ops de lote). El verbo `regressStatus` citado originalmente **no existía** como método de servicio (drift).

> _Original:_ Solo `InvoicePipelineService` llevaba el sufijo `*PipelineService`. Los otros 5 usan `*Service` plano y Advance ni tiene coordinador propio. Verbos divergentes: `saveAndAdvance`/`advance`/`advanceStatus`/`advanceGroup`; `regress`/`regressStatus`/`reject`. Validación divergente: `validateTransitionRequirements`/`getTransitionErrors`/`validateTransition`/`validateGroupTransition`. Advance usa prefijo `AdvanceLegalization*` que contradice Controller/ruta/template `Advances`.
...
- **Novelty: autorización por-paso en `advance`/`reject`/`advanceGroup`** — **✅ EJECUTADO 2026-05-29** (commit `feat(seguridad): enforcement de canOperate en las transiciones de Novelty`). Las 3 transiciones ahora autorizan por paso vía `canOperate` en el servicio (`saveAndAdvance`/`reject`/`advanceGroup` → `denialReasonForAdvance`/`Reject`/`AdvanceGroup`), igual que los otros 4 módulos. La revisión de seguridad adversarial detectó y se **cerró** un mass-assignment en el pre-save de `advanceGroup` (whitelist + gate `canOperate`). `php -l` + 192/192. **Pendiente: verificación manual por rol vía la UI de Permisos** antes de producción. **Plan/detalle: [`docs/plan-seguridad-novelty-pipelineaction.md`](plan-seguridad-novelty-pipelineaction.md).**
  - **CORRECCIÓN (2026-05-29) de la redacción original** ("migrar todo a `#[PipelineAction]`"): tras verificar el canon, eso era impreciso. El canon **mantiene `#[Permission(action:'edit')]` en el save+advance** (`InvoicesController:266`, `RefundsController:311`, `PettyCashRecordsController:247`) y pone la seguridad por-paso en el **servicio** (`saveAndAdvance` → `filterEntityData` + `denialReasonForAdvance`/`canOperate`). `#[PipelineAction]` es solo para transiciones **puras** (`regressStatus`, Refund `advanceStatus`) o atadas a un paso fijo (pagos).
  - Por tanto: `advance` y `advanceGroup` (save+advance) **se quedan en `#[Permission(edit)]`** + ganan `filterEntityData` + `denialReasonForAdvance`/`denialReasonForAdvanceGroup` en el servicio; **solo `reject`** (transición pura) va a `#[PipelineAction(PIPELINE_NOVELTIES)]` + `denialReasonForReject`.
  - El hueco real NO es el atributo: **Novelty es el único módulo que NO hace enforcement de `canOperate` en sus transiciones** (los otros 4 sí, en el servicio). Usa `canOperate` solo para display (`EmployeeNoveltiesController:522`); el enforcement pasa solo por `#[Permission(edit)]`. Además `advance` hace `patchEntity` crudo (sin `filterEntityData`) y no recibe `roleId`. → Hoy un rol con `edit` puede operar cualquier paso sin importar `pipeline_permissions`.
  - **Es puro cambio de código** (servicio + atributos): añadir `denialReasonForAdvance`/`Reject`/`AdvanceGroup` (`canOperate`) + `filterEntityData` + `roleId` a las 3 transiciones. **NO toca BD**: `pipeline_permissions` la gestiona el admin vía la UI "Permisos de Pipeline" (sin seed). Auditoría confirmó que el único bypass hardcodeado por rol es Admin (+ users/roles); ningún servicio de flujo valida por rol exacto.
  - Requiere solo: (1) decisión §5.1 (`reject` autoriza con `canOperate(paso actual)`), (2) decisión §5.2 (`filterEntityData` en `advanceGroup` si el doc edita campos por paso), (3) prueba manual por rol vía la UI de Permisos. Ver plan.
