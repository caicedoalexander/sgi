...
| C5 | **`email_log_panel` solo en Invoice + EN/edit** | ¿Refund/PettyCash/PaymentScheduling/Advance emiten correos de pipeline vía `EmailLogService`? Si sí → A (alinear); si no → B. Asimetría adicional: EN/edit sí, NLD/edit no (mismo `NoveltyPipelineService`) | Ninguno; requiere saber el dominio de notificaciones |
| C6 | **`document_row` server-render directo (sin `documents_section`) en 3 `view`** | `PettyCash/view:394`, `PaymentSchedulings/view:202`, `NLD/view:372` usan el átomo en loop read-only. `documents_section` acopla IDs de `sgi-document-uploader.js` (upload) innecesarios en read-only. ¿Flag `readOnly` en `documents_section`, o aceptar `document_row` directo como canon de `view`? (3 `view` ya convergieron a directo → menor riesgo) | Ninguno |
