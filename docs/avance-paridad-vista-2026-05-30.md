...
- **A1 — el diff celda-a-celda evitó un barrido a ciegas.** El adversarial exigió comparar cada array de pills inline contra su `*Presentation::STATUS_BADGES` antes de unificar. Resultado: solo 2 eran drifts reales (`Invoices/edit` pintaba `verificacion_pago` como `info-soft` ≠ const `warning-soft`; `EmployeeNovelties/view` tenía un mapa stale con `rrhh: info-soft` ≠ `accent-soft` + 3 claves faltantes que caían a `pill-muted`). El resto eran copias idénticas o no-ops redundantes. Alcance real: 8 templates en 4 módulos (Invoice/Refund/Novelty/Advance), ediciones triviales, no un refactor — PettyCash y PaymentScheduling ya leían su const desde antes, así que la cobertura de los 6 módulos se alcanzó **acumulativamente entre olas**, no en Ola 1.
...
|---|---|---|---|
| **A1** | Mapas estado→pill inline → `*Presentation::STATUS_BADGES` (8 templates, 4 módulos) | ✅ ejecutado | `44acb33` — 2 drifts reales corregidos de paso |
...
| **C2** 🔑 | `RowView` generalizado (lógica dentro de `forRow()`) | ✅ ejecutado | `0ab9da5`+`fbb0d1f` — 6 RowViews nuevos (incl. piloto Refund) + Invoice ampliado (P9) |
...
| **C9** | Grids inline de sub-tablas (facturas/pagos) sin element | ⏸ diferido | exigiría crear un element nuevo, ganancia marginal |
