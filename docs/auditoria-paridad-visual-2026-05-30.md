...
1. **El único defecto estructural (A) uniforme es el shell de edición.** Los **6 surfaces de edición** no-Invoice (EN/edit, NLD/edit, Advances/legalization, PettyCash/edit, Refund/edit, PaymentScheduling/edit) están construidos sobre `.sgi-invoice-view-grid` — el grid de la vista **view** — en vez del `.sgi-edit-shell` (app-shell de altura completa: header fijo + columnas con scroll independiente + footer sticky). Confirmado por grep: **`.sgi-edit-shell` es exclusivo de Invoice.** Esto es ~80 % del valor de remediación y es justo lo que el marco "Invoice = canon visual" manda alinear (revirtiendo A9/A10 de la auditoría previa).

2. **`index` y `view` ya están alineados en contenedor en los 6 módulos.** Todos los `index` replican fielmente el patrón `.sgi-card(padding:0)` + grid-header inline + filas `.row-fact` (ancla, no `<table>`) + `.chip` + `.input` + `.pipeline-mini` + `.empty-state` + `element('pagination')`. Todos los `view` usan `.sgi-invoice-view-grid` > `left`+`right` + `element('pipeline_sidebar')`. Reutilización de elements compartidos (pipeline_sidebar, documents_section, pagination, empty-state, pills soft) **ejemplar y uniforme** — en varios casos los módulos reutilizan *más* elements que el propio Invoice.

3. **La cola de divergencias es mayormente cosmética y se colapsa en 2–3 lotes.** Tras las reclasificaciones del adversarial: `.card`→`.sgi-card` (cero delta visual), idioma de tarjeta de detalle a 2 columnas, 2 botones-remove inline, el callout inline de legalization, y `active.php` (gemelo legacy). El resto son **(B) esenciales** (tablas agrupadas de dominio, soportes con firma de Advance, side-rail de calendario de Novelty, forms por-estado) o **(C) decisiones de proyecto**.

4. **Corrección importante al vocabulario del brief:** **Invoice NO usa `.sgi-info-grid`/`.sgi-info-cell`.** Maqueta sus campos de detalle con **`.field-row`** dentro de grids inline `display:grid;grid-template-columns:1fr 1fr;gap:28px` (`Invoices/view.php:213,281`). `.sgi-info-grid` existe en `components.css:2301-2335` pero es un componente **huérfano** que solo consume `EmployeeNovelties/edit.php:178`. El canon de campos de detalle es **`.field-row`**, no `.sgi-info-grid`.
...
|---|---|---|---|---|
| **Invoice** (CANON) | ✅ `.sgi-card`(p:0)+`.row-fact` `index.php:272,293` | 🗑️ `.card.card-primary` `add.php:25` | ✅ **`.sgi-edit-shell`** `edit.php:147` | ✅ `.sgi-invoice-view-grid` `view.php:123` |
| **EmployeeNovelties** | ✅ side-rail `1fr 320px` 🟦 `index.php:207`; lista `.sgi-card`(p:0) `:218`+`.row-fact.head` `:219` | 🗑️ `.card.card-primary` `add.php:38` | ⚠️ `.sgi-invoice-view-grid` `edit.php:131` → shell | ⚠️ `.sgi-invoice-view-grid` `view.php:78` (info-card A `:112`) |
| **NoveltyLiquidationDocs** | ✅ `.sgi-card`(p:0) `index.php:133`+`.row-fact` `:151` (réplica más limpia) | — (sin add 🟦) | ⚠️ `.sgi-invoice-view-grid` `edit.php:92` → shell | ⚠️ `.sgi-invoice-view-grid` `view.php:66` (info-card A `:102`) |
| **Advance** | ✅ `.sgi-card` `index.php:146`+`.row-fact` `:163` (+6ª col Legalización 🟦) | 🗑️ `.card.card-primary` | ⚠️ `legalization.php:88` `.sgi-invoice-view-grid` → shell **(🔶 C1: sin form único)** | ⚠️ `.sgi-invoice-view-grid` `view.php:49` (info-card A `:80`) |
| **PettyCash** (Records) | ✅ `.sgi-card`(p:0) `index.php:203`+`.row-fact` `:223` | 🗑️ `.card.card-primary` | ⚠️ `.sgi-invoice-view-grid` `edit.php:151` → shell (**footer ya presente** `:467`, re-parentar) | ✅ **contraejemplo correcto**: `.sgi-card`+`grid 1fr 1fr;gap:28px` `view.php:170,213` |
| **Refund** | ✅ `.sgi-card`(p:0) `index.php:197`+`.row-fact` `:215` | 🗑️ `.card.card-primary` | ⚠️ `.sgi-invoice-view-grid` `edit.php:103` → shell (**footer ya presente** `:419`) | ⚠️ `.sgi-invoice-view-grid` `view.php:50` (info-card A `:81`; tabla agrupada 🟦 `:136`) |
| **PaymentScheduling** | ✅ `.sgi-card`(p:0) `index.php:170`+`.row-fact` `:192` (filtros reducidos 🟦) | 🗑️ `.card.card-primary` | ⚠️ `.sgi-invoice-view-grid` `edit.php:65` → shell (**footer ya presente** `:253`) | ⚠️ `.sgi-invoice-view-grid` `view.php:46` (info-card A `:77`; tabla 🟦 `:129`) |
...
- **Advance** — `index`/`view` casi perfectos. `legalization.php` es la superficie de trabajo real pero está en el grid de view; **C1**: no hay `<form>` único que envuelva el cuerpo (forms-por-estado), así que el ancla de `.sgi-edit-shell-form` es ambigua. Soportes con firma/consignación bespoke = 🟦 (sancionado en CLAUDE.md).
- **PettyCash** — el más fiel. Su `view` es **el contraejemplo correcto** de tarjeta de detalle (idéntico a Invoice). `edit` ya emite el `.sgi-edit-footer` correcto, solo huérfano fuera del shell → el tier más fácil.
...
Esqueletos canónicos que **todo módulo de flujo** debe replicar. Los `.sgi-*`/componentes salen de `components.css`; los estilos inline marcados son **convención canónica** (Invoice los usa así, copiarlos NO es deriva).
...
- ⚠️ **Deriva (marcar):** inline que **re-implementa un átomo** (pill/botón/card/empty-state hechos a mano), o que **reproduce una clase `.sgi-*`** existente (p.ej. re-derivar el grid de view con `grid-template-columns:340px 1fr` en vez de la clase).
...
SEARCH+FILTROS  Form(get)
  ...
  └─ .collapse > .sgi-card.compact > .row.g-2 (.form-select-sm + .flatpickr-date)
CHIPS   .d-flex[role=tablist] > .chip / .chip.is-active (+ .dot)
TABLA   .sgi-card [style="padding:0"]
...
### 2.3 Esqueleto EDIT (`.sgi-edit-shell`) — **el canon a adoptar en los 5**

```
.sgi-edit-shell                                   (≥992px: position:fixed, viewport completo)
├─ .sgi-edit-shell-head                           (header FIJO, flex-shrink:0, borde inferior)
│    └─ breadcrumb + .sgi-title-page + chip .mono id + .pill estado + acciones .btn-default
├─ Form->create(..., ['class' => 'sgi-edit-shell-form'])   (flex:1, columna flex)
│   └─ .sgi-edit-shell-body  (flex:1; min-height:0; SCROLL)
│       └─ .row.gx-3
│           ├─ <aside class="col-lg-3 sgi-edit-col">   (340px, scroll independiente)
│           │     element('pipeline_sidebar')  (hero + pipeline-v + acciones, vía ob_start)
│           └─ <main class="col-lg-9 sgi-edit-col">    (scroll independiente)
│                 .sgi-card "Datos Generales"  (.sgi-label + .hr + .row.g-2 form)
│                 .sgi-card [position:relative] + .accent-strip "Etapa actual · editable"
│                 element('payment_section') / element('documents_section') / …
└─ .sgi-edit-footer                               (dentro del shell → position:static)
     ├─ .sgi-edit-footer-meta      (Rol + última modificación + dirty indicator)
     └─ .sgi-edit-footer-actions   (.btn-ghost Cancelar + la ÚNICA .btn-primary submit)
```
Referencia: `Invoices/edit.php:147,150,213,216,243,326,887`. Contrato CSS: `components.css:2385-2436`. **Nota:** el split izq/der del shell usa **Bootstrap `.col-lg-3`/`.col-lg-9`** (cada uno también `.sgi-edit-col`), NO el `340px 1fr` que usa el *view*.

### 2.4 Esqueleto VIEW (`.sgi-invoice-view-grid`) — ya alineado en contenedor

```
HEADER  breadcrumb + .sgi-title-page (h1) + acciones .btn-default/.btn-secondary
.sgi-invoice-view-grid          (grid 340px 1fr; gap:14px)
├─ <aside class="sgi-invoice-view-left">   element('pipeline_sidebar')
└─ <main class="sgi-invoice-view-right">
     .sgi-card "Datos generales"
        └─ <div [inline grid 1fr 1fr; gap:28px]>  .sgi-label + .field-row (.k/.v/.v.mono)
     .sgi-card pagos/observaciones/soportes/historial
        (docs: element('documents_section') read-only · pills .pill-*-soft · .av · .empty-state)
```
Referencia: `Invoices/view.php:123,126,209,213,281`. **Canon de campos = `.field-row` dentro de grid inline `1fr 1fr;gap:28px`** (NO `.sgi-info-grid`).

### 2.5 Vocabulario de átomos (nunca hacer a mano)

Botones `.btn` (+`-primary/-default/-ghost/-secondary/-danger/-subtle/-sm/-icon`) · Pills `.pill` (+`-*-soft`/`-sm`/`-lg`/`.pill-dark`/`.pill-orange`) · Inputs `.input` (+`.input-label`/`.input-help`) · `.av` (`-sm/md/lg/xl`) · `.doc` · `.chip` (+`.is-active`/`.dot`) · `.tab`/`.tab-badge` · `.segmented`/`.seg` · `.field-row` (`.k`/`.v`/`.v.mono`) · `.pipeline-mini`/`.pipeline-v` · `.empty-state`/`.es-*` · `.dropzone`/`.dz-*` · `.bank-chip` · `.sgi-card`(+`.compact`)/`.sgi-section-head`/`.sgi-label`/`.mono`/`.hr`/`.accent-strip`/`.banner`/`.sgi-folder-count`. Header: breadcrumb + `.sgi-title-page`/`.sgi-page-header`/`.sgi-edit-id-chip` (**ver C2**).
...
|---|---|---|---|---|
| **A1** 🔑 | **EDIT sobre `.sgi-invoice-view-grid` en vez de `.sgi-edit-shell`** | TODOS los 5: `EmployeeNovelties/edit.php:131`, `NoveltyLiquidationDocs/edit.php:92`, `Advances/legalization.php:88`, `PettyCashRecords/edit.php:151`, `Refunds/edit.php:103`, `PaymentSchedulings/edit.php:65` | Envolver en `.sgi-edit-shell` (head/form/body/`.col-lg-3+9 .sgi-edit-col`/footer) | **Alta — único A estructural** |
| A2 | Tarjeta de detalle a 2 col con `.card`+`.row.g-0`+`.col-md-6`+`border-right` vs Invoice `.sgi-card`+grid inline `1fr 1fr;gap:28px`+`.field-row` | `EmployeeNovelties/view.php:112`, `NoveltyLiquidationDocs/view.php:102`, `Advances/view.php:80`, `Refunds/view.php:81`, `PaymentSchedulings/view.php:77` (PettyCash exento — lo hace bien `view.php:170`) | Adoptar `.sgi-card`+grid inline `1fr 1fr;gap:28px`. Sancionado por `components.css:1116` → baja prioridad | Baja (cosmética) |
| A3 | `.card`+`style="padding:18px 20px"` re-implementa `.sgi-card` | `EmployeeNovelties/edit.php:174,271`, `NoveltyLiquidationDocs/edit.php:150,195,317`, `PaymentSchedulings/edit.php:116` | `.sgi-card` (quitar padding inline); `.sgi-card.compact` si denso | Baja (cero delta visual) |
| A4 | `active.php` legacy: `.card.card-primary`/`.card-dark` + `.btn-outline-*` scope nav | `EmployeeNovelties/active.php:16-24,28,51` | `.sgi-card.compact`/`.sgi-card`(p:0) + `.chip` tabs (como su propio `index.php:149`), o **retirar** a favor del `index` modo `all` | Media |
| A5 | Botón-remove icon-only inline (`.btn-outline-danger`+`style="padding:.15rem .4rem"`) | `Refunds/edit.php:265`, `PaymentSchedulings/edit.php:197` | `.btn.btn-icon`+`.sgi-fg-danger` ó `.btn.btn-danger.btn-sm` | Baja (2 swaps) |
...
| A8 | `.sgi-edit-footer` huérfano fuera del shell → variante page-fixed | `PettyCashRecords/edit.php:467`, `Refunds/edit.php:419`, `PaymentSchedulings/edit.php:253` | **Se resuelve solo con A1** (re-parentar dentro del shell → `position:static`) | — (consecuencia de A1) |
...
| B2 | **Soportes con firma/consignación bespoke** (`Advances/legalization.php:459-624`; firmas en NLD `edit.php:199`, `view.php:189`) | Docs con pills firmado/pendiente + reemplazo AJAX `fetch()` fuera del contrato `document_row`↔`sgi-document-uploader.js` — sancionado en CLAUDE.md. Reutilizan correctamente `.doc-row`/`.doc-icon` |
| B3 | **Side-rail de calendario `1fr 320px` + swatch de color de tipo** (`EmployeeNovelties/index.php:207,271`; `.sgi-calendar`) | Dimensión temporal/calendario que los otros listados no tienen; el swatch es la clave-de-color del evento (NO un status pill — sería violación de "pills soft") |
...
| B10 | **Header helper-set `.sgi-page-header`/`.sgi-breadcrumb`/`.sgi-edit-id-chip`** (11 templates) | Son clases canónicas reales (systematización de `styles.css`), NO deriva ad-hoc — pero ver C2 |
...
|---|---|---|---|
| **C1** | **Ancla de `.sgi-edit-shell-form` en `Advances/legalization`** | No hay `<form>` único que envuelva el cuerpo (forms-por-estado, `legalization.php:234-454`). La clase es solo un *hook flex*, así que un `<div class="sgi-edit-shell-form">` no-form es técnicamente limpio | Aplicar la clase a un `<div>` envolvente (las forms por-estado quedan anidadas), o variante de shell bespoke. **El resto del shell (head/body/col/footer) aplica sin cambios** |
| **C2** | **Idioma de header (+ homógrafo + `.btn-ghost-card`)** | Dos idiomas canónicos: helper-set `.sgi-page-header`/`.sgi-breadcrumb`/`.sgi-page-title`/`.sgi-edit-id-chip` (11 templates) vs inline `.sgi-title-page` (Invoice). **Invoice es internamente inconsistente** (view≠edit) → "alinear a Invoice" es ambiguo. Además `.sgi-title-page` vs `.sgi-page-title` son near-homógrafos del mismo h1 | Elegir UN idioma + UN token de título y converger los 12 templates; **matar uno de los dos homógrafos**. (Por adopción/abstracción, el helper-set es el target más fuerte; pero por el marco "Invoice=canon" requiere tu visto bueno.) `.btn-ghost-card` vs `.btn-default` (A10) se pliega aquí |
| **C3** | **Idioma de campos de detalle** | CUATRO idiomas conviven: (1) Invoice inline-grid+`.field-row` (canon), (2) `.row.g-0`+`border-right`+`.field-row` (5 views, A2, sancionado), (3) `.sgi-info-grid`/`.sgi-info-cell` huérfano (solo EN/edit `:178`), (4) copia correcta de PettyCash/view | Estandarizar en (1) `.field-row`+grid inline (canon real). **Decidir si se deprecia `.sgi-info-grid`/`.sgi-info-cell`** (`components.css:2301-2335`) o se promueve a canon (migrando Invoice a él, lo que invierte la dirección) |

### Falsos positivos descartados por el adversarial (no son trabajo)

- **`.sgi-info-grid` ≠ canon** — el brief lo listaba como canon de view; verificado: Invoice usa `.field-row`. `.sgi-info-grid` de EN/edit es un *tercer* idioma visualmente distinto (4-col con bordes), **no** "canon correcto".
- **`.card` vs `.sgi-card`** — ambos `border:0;radius:0;box-shadow:none` → **cero delta visual**. Solo higiene de token (A3, "renombrar al pasar por el archivo").
- **Refund "Facturas Agrupadas"** — es tabla 5-col con pill de pipeline (B1), no lista plana → **no** convertir a `.row-flex`.
- **Swatch de tipo / `.doc-row`/`.doc-icon` / hooks `.sgi-beneficiary-*`** — color-key de calendario / reuso correcto de clase canon / hooks JS. No son re-implementaciones de átomos.
...
### Ola 1 — Migración al `.sgi-edit-shell` (≈80 % del valor; A1 + A8)

El único defecto estructural. **3 tiers por dificultad:**

1. **Fácil — PettyCash, Refund, PaymentScheduling.** Ya emiten el `.sgi-edit-footer` correcto (`PettyCashRecords/edit.php:467`, `Refunds/edit.php:419`, `PaymentSchedulings/edit.php:253`); solo hay que envolver en `.sgi-edit-shell`, mover el header a `.sgi-edit-shell-head`, poner `.sgi-edit-shell-form` en el `Form->create`, renombrar el body a `.sgi-edit-shell-body` con `.col-lg-3/9 .sgi-edit-col`, y re-parentar el footer dentro del shell (A8 se resuelve gratis → `position:static`).
2. **Medio — EmployeeNovelties/edit, NoveltyLiquidationDocs/edit.** Sin footer; advance es **por-estado** (B4). Consolidar los submit/avanzar en `.sgi-edit-footer-actions` **preservando los cuerpos de form por-estado** en el body.
...
- **A2** — tarjeta de detalle a `.sgi-card`+grid inline `1fr 1fr;gap:28px` en los 5 views (← **decidir C3 primero**).
- **A3** — `.card`+padding → `.sgi-card` (al tocar cada edit en Ola 1).
- **A5** — 2 botones-remove → `.btn-icon`/`.btn-danger`.
- **A6** — callout legalization → `.banner.info`.
- **A4** — `active.php` → `.chip` + `.sgi-card` (o retirarlo).
...
- **C3** — resolver idioma de campos de detalle (deprecar o promover `.sgi-info-grid`).
...
1. **C1** — ¿`.sgi-edit-shell-form` sobre un `<div>` envolvente en legalization (las forms por-estado anidadas), o variante de shell bespoke?
2. **C2** — ¿idioma de header único? Helper-set `.sgi-page-header` (11 templates, más DRY) **o** inline `.sgi-title-page` (Invoice). Y ¿cuál de los homógrafos `.sgi-title-page`/`.sgi-page-title` se elimina?
3. **C3** — ¿campos de detalle en `.field-row`+grid inline (canon real de Invoice) y **deprecar** `.sgi-info-grid`/`.sgi-info-cell`? ¿O promover `.sgi-info-grid` a canon (migrando Invoice)?
