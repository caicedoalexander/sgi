# Sistema de Diseño SPI · COPCSA — Reglas, copy y convenciones
...
3. **Datos cuantitativos en mono** — `JetBrains Mono` es OBLIGATORIO para códigos de factura, fechas, montos, IDs, cédulas, nombres de archivo. Aplica con `.spi-mono` o `.mono`.
4. **Labels en uppercase** — etiquetas de sección y columnas de tabla: 10–11px / 700 / `letter-spacing 0.6–0.8`. Usa `.spi-label` o `.label-up`.
...
El SPI es una herramienta operativa para administrativos y supervisores. Tono **directo, profesional, español formal pero no rígido**.
...
### Chat bubbles (`.spi-obs-bubble-body`)

```css
.spi-obs-bubble.is-mine  .spi-obs-bubble-body { border-radius: 10px 10px 2px 10px; }
.spi-obs-bubble.is-other .spi-obs-bubble-body { border-radius: 10px 10px 10px 2px; }
```
Radio asimétrico 10/2 es convención universal (WhatsApp/Slack/iMessage). Mantiene 2px como ancla al sistema.

### Counter pills (`.spi-folder-count`)

```css
.spi-folder-count { border-radius: 10px; padding: .15em .5em; min-width: 1.5em; }
```
...
<!-- 5. Componentes SPI (debe ir después de Bootstrap para overrides) -->
...
| `webroot/js/spi-common.js` | Auto-init: Flatpickr, AutoNumeric, Select2, clickable rows |
...
| Sin prefijo | `spi-` prefijado |
|-------------|------------------|
| Componentes UI: `.btn`, `.pill`, `.input`, `.av`, `.doc`, `.cal`, `.row-fact`, `.pipeline-mini`, `.sb-item`, `.tab`, `.chip` | Utilidades / tipografía: `.spi-card`, `.spi-display`, `.spi-title-page`, `.spi-title-card`, `.spi-label`, `.spi-mono`, `.spi-body-muted`, `.spi-body-faint`, `.spi-fg-*` |

La regla: **componentes** = sin prefijo (más legible en markup); **utilidades de marca/sistema** = `spi-` para no chocar con Bootstrap o librerías de terceros.
