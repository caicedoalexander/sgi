...
- **2026-05-06** — Cierre de deudas menores: (1) `confirmShortageReceipt` deja de usar `DocumentUploadTrait` directo — se agregó `AdvanceLegalizationDocumentService::attachShortageReceipt` y `AdvanceLegalizationService` ya no consume el trait (eliminado `use DocumentUploadTrait`). (2) `PettyCashRecordsController::_buildSyntheticPayments` migró a `PettyCashService::buildSyntheticPayments` retornando `BulkPaymentView[]` — paridad con `RefundService`. Auditoría sin deudas pendientes.
- **2026-05-06** — Excepción registrada a regla firme §5: `InvoicePipelineService` conserva el sufijo `Pipeline` en lugar de migrar a `InvoiceService`. Razón: (a) Invoice fue el primer dominio en adoptar el State pattern (ver ADR 0006 — `state-pattern-invoice-pipeline`) y el nombre comunica esa decisión arquitectónica; (b) renombrar afectaría 25 archivos incluyendo `ADR 0006`, `architecture.md` y la auditoría arquitectónica, churn desproporcionado al valor de la consistencia nominal. La regla firme §5 sigue aplicando para los demás dominios. Si el archivo se toca por otro motivo en el futuro, evaluar el rename en esa sesión.
- **2026-05-06** — Asimetría justificada en `NoveltyLiquidationDoc`: existe `NoveltyLiquidationDocEditViewModel` sin `…AddViewModel`. Razón: `NoveltyLiquidationDocsController` no tiene método `add()` — los liquidation docs se crean automáticamente vía `LegalizationInitializerSubscriber` cuando una novedad alcanza el estado correspondiente. La única vía de entrada del usuario es `edit()`, por eso solo existe Edit VM. La regla firme §5 sobre simetría Add/Edit aplica donde hay creación por UI; no aplica aquí.
- **2026-05-06** — Verificación final: tras revisión estructural, los 8 puntos del audit están cumplidos y las deudas residuales quedan registradas como excepciones justificadas. Auditoría definitivamente cerrada.
