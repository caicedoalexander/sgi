 ...
 * `listSelector` (debe contener un `.spi-folder-count`).
 *
...
(function (global) {
    ...
    var DOC_ICON_RULES = (function () {
        ...
        try {
            var el = document.getElementById('spi-doc-icon-rules');
            if (el) {
    ...
    // ─── Flash notification (delega en SpiToast, fuente única de toasts) ─────
    function showToast(message, variant) {
        if (global.SpiToast) {
            global.SpiToast.show(message, variant || 'danger');
            return;
        }
        global.alert(message);
    }

    // Delegado en SpiDialogs (spi-dialogs.js). Mantenemos un fallback local
    // por si este script se carga sin el módulo de diálogos.
    function confirmDialog(message) {
        if (global.SpiDialogs && global.SpiDialogs.confirm) {
            return global.SpiDialogs.confirm(message);
        }
    ...
    function init(opts) {
        ...
        var counter      = listCard
            ? listCard.querySelector('.spi-folder-count')
        ...
        if (form) {
            form.addEventListener('submit', function (e) {
                ...
                // SPI_MAX_UPLOAD_BYTES/LABEL los expone spi-common.js leyendo
                // <meta name="spi-max-upload-{bytes,label}"> inyectado por default.php.
                // Adicionalmente, spi-common.js tiene un submit-listener global con
                ...
                var maxBytes = global.SPI_MAX_UPLOAD_BYTES || (20 * 1024 * 1024);
                var maxLabel = global.SPI_MAX_UPLOAD_LABEL || '20 MB';
    ...
    global.SpiDocumentUploader = { init: init };
})(window);
