/**
 * SpiObservationChat
 * Helper unificado para chats de observaciones en módulos SPI
 ...
 *   });
 *
 * El contador (`.spi-folder-count`) se resuelve en este orden:
 *   1. `counterSelector` si se pasa explícito (recomendado cuando hay
 *      varios `.spi-folder-count` en la misma pantalla, ej. docs + obs).
...
(function (global) {
    ...
    function ensureCounter(listCard) {
        ...
        var existing = listCard.querySelector('.spi-folder-count');
        if (existing) return existing;
        var header = listCard.querySelector('.card-header');
        if (!header) return null;
        var badge = document.createElement('span');
        badge.className = 'spi-folder-count ms-auto';
        badge.textContent = '0';
        ...
        form.addEventListener('submit', function (e) {
            ...
            .catch(function (err) {
                if (global.console && global.console.error) {
                    global.console.error('SpiObservationChat:', err);
                }
    ...
    global.SpiObservationChat = { init: init };
})(window);
