/**
 * SPI Payment Section — shared JS module.
 *
...
(function () {
    ...
    function init() {
        document.querySelectorAll('[data-payment-section]').forEach(function (section) {
            ...
            if (!addUrl) {
                console.error('[spi-payment] Falta data-add-url en la sección de pagos.');
                return;
            }
            ...
            if (!bankInput || !amountInput || !dateInput) {
                console.error('[spi-payment] Faltan inputs de pago (bank/amount/date).');
                return;
            }
            ...
            // 2. El input lleva class="currency-input" que spi-common.js inicializa
            //    en DOMContentLoaded vía spiInitDynamic.
            ...
            function validate() {
                ...
                if (!bankInput.value || !raw || parseFloat(raw) <= 0 || !dateInput.value) {
                    window.SpiDialogs.toast('Complete todos los campos del pago (entidad, monto y fecha).', 'warning');
                    return false;
                }
                return true;
            }
            ...
            btnRegisterAdvance.addEventListener('click', function (e) {
                e.preventDefault();
                if (!validate()) return;
                window.SpiDialogs.confirm('Este pago se registrará y la factura pasará inmediatamente al estado de Autorización de Pago. ¿Continuar?').then(function (ok) {
                    if (!ok) return;
