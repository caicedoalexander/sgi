/**
 * SPI Common JS - Flatpickr + AutoNumeric + Row click + Modal AJAX loader
 */
...
(function () {
    var metaBytes = document.querySelector('meta[name="spi-max-upload-bytes"]');
    var metaLabel = document.querySelector('meta[name="spi-max-upload-label"]');
    window.SPI_MAX_UPLOAD_BYTES = metaBytes ? parseInt(metaBytes.content, 10) : (20 * 1024 * 1024);
    window.SPI_MAX_UPLOAD_LABEL = metaLabel ? metaLabel.content : '20 MB';
})();

// Global guard: bloquea el submit de cualquier formulario con un archivo que supera
// SPI_MAX_UPLOAD_BYTES para evitar el 413 de nginx. capture:true asegura que se ejecute
...
document.addEventListener('submit', function (e) {
    ...
    var max = window.SPI_MAX_UPLOAD_BYTES;
    var label = window.SPI_MAX_UPLOAD_LABEL;
...
// Listener delegado para botones con `data-spi-confirm="mensaje"`. Reemplazo
// del patrón `onclick="return confirm(...)"` inline — usa el modal estilizado
// de SpiDialogs cuando está disponible, con fallback al confirm() nativo.
...
document.addEventListener('click', function (e) {
    var btn = e.target.closest('[data-spi-confirm]');
    if (!btn) return;
    if (btn.dataset.spiConfirmConfirmed === '1') {
        delete btn.dataset.spiConfirmConfirmed;
        return;
    }
    e.preventDefault();
    e.stopPropagation();
    var message = btn.getAttribute('data-spi-confirm') || '¿Confirmar?';
    var go = (window.SpiDialogs && window.SpiDialogs.confirm)
        ? window.SpiDialogs.confirm(message)
        : Promise.resolve(window.confirm(message));
    go.then(function (ok) {
        if (!ok) return;
        btn.dataset.spiConfirmConfirmed = '1';
        btn.click();
    });
}, true);
...
window.spiInit = function (root) {
    ...
    if (typeof $ !== 'undefined' && $.fn && $.fn.select2) {
        $(root).find('select.select2-enable').each(function () {
            ...
            // dropdownParent: si el select vive dentro de un modal o del drawer
            // de filtros, el desplegable debe anclarse ahí (z-index/foco), no en body.
            var $container = $(this).closest('.modal, .filter-drawer');
            $(this).select2({
                width: '100%',
                language: 'es',
...
/**
 * SpiToast — toasts del Sistema de Diseño (icon-box + título + mensaje + cierre
 ...
 *
 *   SpiToast.show('Mensaje', 'success'|'danger'|'warning'|'info', { title, icon, autoDismiss });
 */
...
function bindToast(toast) {
    ...
    // Reanuda el conteo (y la barra) por el tiempo restante. Llamado al inicio
    // y cada vez que el mouse sale del toast.
    function resume() {
        startedAt = Date.now();
    ...
    // Pausa al pasar el mouse: descuenta lo ya transcurrido y congela la barra.
    function pause() {
        clearTimeout(timer);
...
function initToasts(root) {
    (root || document).querySelectorAll('#spi-flash-container .toast').forEach(bindToast);
}

window.SpiToast = { show: showToast, bind: bindToast };

document.addEventListener('DOMContentLoaded', function () {
    window.spiInit(document);
    ...
    // ── Filter drawer (panel lateral de filtros) ────────────────────────────
    // Abre/cierra el .filter-drawer (uno por página). El panel vive dentro del
    // <form> GET del índice; el submit del footer recarga con los filtros.
    (function () {
        ...
        function openDrawer() {
            ...
            var first = drawer.querySelector('select, input:not([type="hidden"]), textarea, button');
